...
import com.jeebase.system.webchat.dto.TokenUser;
import com.jeebase.system.webchat.dto.WebChatLoginUser;
...
public class WeChatLoginController {
    ...
    @Autowired
    private ISmsService ISms;
    
    @GetMapping("/code")
    @NoAuthentication
    @ApiOperation(value = "获取微信登录url", notes = "获取微信登录url")
    public Result<String> code(String code) throws Exception {
    	//System.out.println(code);
        return new Result<String>().success().put(code);
    }
    ...
    @PostMapping("/readCardImg")
    @NoAuthentication
    @ApiOperation(value = "身份证卡片信息", notes = "返回身份证信息")
    public Result<ImgIDCardInfo> readIdCardImg(MultipartFile front,MultipartFile back){
    	ImgIDCardInfo backImgInfo = this.ISms.idCardImgDiscern(back, "back");
    	ImgIDCardInfo frontImgInfo = this.ISms.idCardImgDiscern(front, "front");
    	if(backImgInfo!=null&&frontImgInfo!=null) {
    		BeanCopier.create(ImgIDCardInfo.class, ImgIDCardInfo.class, false).copy(frontImgInfo, backImgInfo, null);
    		return new Result<ImgIDCardInfo>().put(backImgInfo).success("成功");
    	}
    	if(backImgInfo==null) {
    		return new Result<ImgIDCardInfo>().error("反面证件图片无效");
    	}
    	if(frontImgInfo==null) {
    		return new Result<ImgIDCardInfo>().error("正面证件图片无效");
    	}
    	return new Result<ImgIDCardInfo>().error("失败");
    }
    ...
    public Result<TokenUser> login( String code, HttpServletRequest request) throws Exception {
        ...
        if(null == user)
        {
            ...
            addUser.setGender(Long.parseLong(snsUserInfo.getSex()));
            ...
            try
            {
                ...
                BeanCopier.create(SnsUserInfo.class, Wechat.class, false).copy(snsUserInfo,
                        wechat, null);
                wechat.setSex(Long.parseLong(snsUserInfo.getSex()));
        ...
        //更新为最新的用户信息
            ...
            if(user.getUserId()!=null)
            {
                ...
                updateUser.setContry(snsUserInfo.getCountry());
                updateUser.setProvince(snsUserInfo.getProvince());
                updateUser.setCity(snsUserInfo.getCity());
                updateUser.setWechatPlatformOpenId(snsUserInfo.getOpenid());
                updateUser.setWechatUnionId(snsUserInfo.getUnionid());
                updateUser.setGender(Long.parseLong(snsUserInfo.getSex()));
                
                wechatMemberService.updateById(updateUser);
                // 查询用户是否存在
                User userQ = userService.getById(user.getUserId());
                String token = jwtComponent.sign(userQ.getUserAccount(), userQ.getUserPassword(), Constant.ExpTimeType.APP);
                tokenUser.setReg(true);
                tokenUser.setRemember(true);
                tokenUser.setOpenId(snsUserInfo.getOpenid());
    ...
    public Result<TokenUser> usernameLogin(@RequestBody WebChatLoginUser loginUser) throws Exception {
        ...
        ew.eq("user_account", userAccount).or().eq("user_mobile", userAccount).or().eq("user_email", userAccount).isNull("type");
